...
object ImplementationValidator {
  ...
  def validate(program: ResolvedProgram, errors: ErrorSink): Unit = {
    val defined = program.methodDefinitions.toSeq.map(_.name).toSet
    ...
    def expression(expr: ResolvedExpression): Unit = {
      ExpressionVisitor.visit(expr, _ match {
        case invoke: ResolvedInvoke if invoke.method.isDefined && !defined.contains(invoke.method.get.name) => {
