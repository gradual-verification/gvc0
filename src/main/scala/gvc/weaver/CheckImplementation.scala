...
object CheckImplementation {
  ...
  def expression(exp: vpr.Exp, method: Method, returnValue: Option[Expression]): Expression = {
    ...
    exp match {
      case eq: vpr.EqCmp => new Binary(BinaryOp.Equal, expr(eq.left), expr(eq.right))
      case ne: vpr.NeCmp => new Binary(BinaryOp.NotEqual, expr(ne.left), expr(ne.right))
      case lt: vpr.LtCmp => new Binary(BinaryOp.Less, expr(lt.left), expr(lt.right))
      case lte: vpr.LeCmp => new Binary(BinaryOp.LessOrEqual, expr(lte.left), expr(lte.right))
      case gt: vpr.GtCmp => new Binary(BinaryOp.Greater, expr(gt.left), expr(gt.right))
      case gte: vpr.GeCmp => new Binary(BinaryOp.GreaterOrEqual, expr(gte.left), expr(gte.right))

      case and: vpr.And => new Binary(BinaryOp.And, expr(and.left), expr(and.right))
      case or: vpr.Or => new Binary(BinaryOp.Or, expr(or.left), expr(or.right))

      case add: vpr.Add => new Binary(BinaryOp.Add, expr(add.left), expr(add.right))
      case sub: vpr.Sub => new Binary(BinaryOp.Subtract, expr(sub.left), expr(sub.right))
      case mul: vpr.Mul => new Binary(BinaryOp.Multiply, expr(mul.left), expr(mul.right))
      case div: vpr.Div => new Binary(BinaryOp.Divide, expr(div.left), expr(div.right))
