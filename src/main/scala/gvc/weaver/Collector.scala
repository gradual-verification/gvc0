...
object Collector {
  ...
  case class TrackedCondition(id: scala.Int, location: Location, value: CheckExpression, when: TrackedDisjunction)
  case class TrackedConjunction(values: List[(TrackedCondition, Boolean)])
  case class TrackedDisjunction(cases: List[TrackedConjunction]) extends Condition
  ...
  private def collect(
    ...
    def check(node: vpr.Node, loc: Location): Unit = {
      ...
      for (check <- vprChecks.get(node).toSeq.flatten) {
        ...
        Check.fromViper(check, irMethod) match {
          case acc: AccessibilityCheck if !node.contains(check.context) => {
    ...
    def convertConjunction(conjunction: Logic.Conjunction): TrackedConjunction =
      TrackedConjunction(conjunction.terms.toSeq.sorted.map(t => (getCondition(t.id), t.value)).toList)
  ...
  private def resolveValue(
    ...
    input match {
      ...
      case n: Var => arguments match {
        case None => CheckExpression.Var(n.name)
        case Some(arguments) => n match {
          case p: Parameter => arguments.getOrElse(p, throw new WeaverException(s"Unknown parameter '${p.name}'"))
          case v => throw new WeaverException(s"Unknown variable '${v.name}'")
        }
      }

      case n: FieldMember => CheckExpression.Field(resolve(n.root), n.field.struct.name, n.field.name)
      ...
      case n: Conditional => CheckExpression.Cond(resolve(n.condition), resolve(n.ifTrue), resolve(n.ifFalse))
      case n: Binary => {
        ...
        n.operator match {
          ...
          case BinaryOp.NotEqual => CheckExpression.Not(CheckExpression.Eq(l, r))
