...
object Bench {
  ...
  private def markExecution(
    ...
    for (_ <- 1 to timedIterations) {
      ...
      val exitCode = (outputCommand #> os) !

      val stop = System.nanoTime()

      os.close()
      if (exitCode != 0) throw new ExecutionException(os.toString())
      duration += stop - start
    }
    duration / timedIterations
  }
  ...
  def run(
    ...
    for (sampleIndex <- 0 until sampling.nSamples) {
      ...
      for (labelIndex <- 0 to sampleToPermute.length - 2) {
        val currentID = PermutationID(sampleIndex + 1, labelIndex + 1)
        printProgress(
        ...
        if(!potentiallyExists.isDefined) {
          ...
          try {
            ...
            if(averageVerificationTime == 0){
              averageVerificationTime = performance.verification
            }else{
              averageVerificationTime = (averageVerificationTime + performance.verification) / 2
            }

            if(averageExecutionTime == 0){
              averageExecutionTime = performance.execution
            }else{
              averageExecutionTime = (averageExecutionTime + performance.execution) / 2
            }

            csv.logSuccess(currentID, performance, permutationHash)
          }catch{
            case exec: ExecutionException => {
              nExecutionFailures += 1
              csv.logFailure(currentID, exec.getMessage, ErrorTypes.Execution)
            }
            case ver: VerificationException => {
              nVerificationFailures += 1
              csv.logFailure(currentID, ver.getMessage, ErrorTypes.Silicon)
            }
            case cc0: CC0Exception => {
              nCompilationFailures += 1
              csv.logFailure(currentID, cc0.getMessage, ErrorTypes.CC0)
            }
          }
        }else
          csv.logSuccess(currentID, potentiallyExists.get.metrics, permutationHash)
      }
    }
  }
  ...
  def printProgress(
    ...
    val compileFailurePercentage = (nCompileFailures) / baseline * 100
    val failurePercentage =
      verFailurePercentage + execFailurePercentage + compileFailurePercentage
    ...
    print(
      s"\rBench: ${sourceFileName} * Path ${currentPath}/${maxPath} * Perm. ${currentIndex}/${maxIndex} * Fails: ${round(
        failurePercentage
      )}% - (V: ${round(verFailurePercentage)}% + C: ${round(compileFailurePercentage)} +  E: ${round(execFailurePercentage)}%) * Ver. time: ${round(
