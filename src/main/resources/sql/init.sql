...
SELECT version_name, measurement_type, src_filename, error_type, error_count
FROM (SELECT version_id, tblA.program_id, tblB.id, total_completed, total_perms
      FROM (SELECT program_id, COUNT(permutations.id) as total_perms
            FROM permutations
                     INNER JOIN programs p on permutations.program_id = p.id

            GROUP BY program_id) as tblA
               INNER JOIN (SELECT program_id,
                                  version_id,
                                  dmt.id,
                           ...
                           GROUP BY program_id,
                                    version_id,
                                    dmt.id) as tblB
                          on tblA.program_id = tblB.program_id) as completion
         LEFT OUTER JOIN
     (SELECT version_id, program_id, dmt.id, errors.error_type, COUNT(DISTINCT error_id) as error_count
      FROM errors
               INNER JOIN dynamic_performance dp on errors.id = dp.error_id
               INNER JOIN permutations ON dp.permutation_id = permutations.id
               INNER JOIN dynamic_measurement_types dmt on dp.dynamic_measurement_type = dmt.id
               INNER JOIN programs p on permutations.program_id = p.id

      GROUP BY version_id, program_id, measurement_type, errors.error_type) as errors
     ON completion.id = errors.id AND completion.program_id = errors.program_id AND
        completion.version_id = errors.version_id
         INNER JOIN versions ON errors.version_id = versions.id
         INNER JOIN dynamic_measurement_types ON completion.id = dynamic_measurement_types.id
         INNER JOIN programs ON completion.program_id = programs.id;
