...
if (length(args) == 0) {
    stop("A result directory must be provided.", call. = FALSE)
}

dir <- args[1]

print(paste("Compiling Directory", (dir), "]---", sep = " "))

path_index_path <- file.path(dir, "path_index.csv")
path_index <- read_csv(
    path_index_path,
    show_col_types = FALSE
)

program_index_path <- file.path(dir, "program_index.csv")
program_index <- read_csv(
    program_index_path,
    show_col_types = FALSE
)

measurement_type_index_path <- file.path(dir, "measurement_type_index.csv")
mt_index <- read_csv(
...
static_performance_path <- file.path(dir, "static_perf.csv")
static_performance <- read_csv(
    static_performance_path,
    show_col_types = FALSE
)

dynamic_performance_path <- file.path(dir, "dynamic_perf.csv")
...
pg <- perf_global %>% write.csv(
        file.path(dir, "compiled_dynamic_performance.csv"),
        row.names = FALSE
)

tg <- table_global %>% write.csv(
        file.path(dir, "compiled_table.csv"),
        row.names = FALSE
)

jg <- jumps_global %>% inner_join(path_index, by=c("program_id","permutation_id","path_id","level_id")) %>%
    write.csv(
        file.path(dir, "compiled_jumps.csv"),
        row.names = FALSE
    )
